...
subject_df <- myData %>%
  ...
  summarize(
    ...
    Total_duration_of_fixations = mean(total_fix_dur_ms),
    Average_duration_of_fixations = mean(avg_fix_dur_ms),
    Number_of_fixations = mean(fix_num)
  )
...
study_only_norm_long <- study_subject_norm_long[1:392, ]
study_only_norm_log_long <- study_subject_norm_long[393:784, ]

retrieval_only_norm_long <- retrieval_subject_norm_long[1:392, ]
retreival_only_norm_log_long <- retrieval_subject_norm_long[393:784, ]

# make group for only LM, Wall, Other, and Objects categories during study and retrieval separately
study_lm_wall_other <- study_subject_norm_long[c(1:28, 85:112, 169:196, 365:392), ]
study_lm_wall_other_log <- study_subject_norm_long[c(393:420, 477:504, 561:588, 757:784), ]

retrieval_lm_wall_other <- retrieval_subject_norm_long[c(1:28, 85:112, 169:196, 365:392), ]
retrieval_lm_wall_other_log <- retrieval_subject_norm_long[c(393:420, 477:504, 561:588, 757:784), ]

# make a group for successive fixations: lm_lm, obj_diff obj, lm_obj_lm,
study_successive_fix <- study_subject_norm_long[281:364, ]
retrieval_successive_fix <- retrieval_subject_norm_long[281:364, ]
...
shapiro.test(subject_df$s.other_norm_log) # marginal
...
shapiro.test(subject_df$r.landmarks_norm) # sig
...
shapiro.test(subject_df$r.DODW_norm_log) # very sig
...
### test study against test in each category # all sig obvi
...
cor.test(subject_df$s.DOSW_norm_log, subject_df$placement_error_cm_log, method = "pearson") # marginal
...
cor.test(subject_df$s.obj_to_lm_norm_log, subject_df$placement_error_cm_log, method = "pearson") # .053 marginal
cor.test(subject_df$s.lm_to_obj_norm_log, subject_df$placement_error_cm_log, method = "pearson") # sig .01
cor.test(subject_df$s.obj_to_so_norm_log, subject_df$placement_error_cm_log, method = "pearson")
...
cor.test(subject_df$s.lm_obj_lm_norm_log, subject_df$placement_error_cm_log, method = "pearson") # sig .027
...
cor.test(subject_df$Total_duration_of_fixations, subject_df$placement_error_cm_log, method = "pearson")
cor.test(subject_df$Average_duration_of_fixations, subject_df$placement_error_cm_log, method = "pearson") # marginal at .0997
...
##### Get rid of subjects with Nas for now
noNAN_subject_df <- na.omit(subject_df)

# big regression with all sig ones to see which explains more variance
# doesn't work yet because there's more terms than subjects
...
small_reg_terms <- subject_df %>%
  dplyr::select("s.landmarks_norm_log", "s.DOSW_norm_log", "s.DODW_norm_log", "s.lm_obj_lm_norm_log", "Average_duration_of_fixations")

cor(small_reg_terms, use = "complete.obs")
pairs(small_reg_terms)

# this is the correct regression
small_reg_norm <- lm(formula = placement_error_cm_log ~ s.landmarks_norm_log + s.DOSW_norm_log + s.DODW_norm_log + s.lm_obj_lm_norm_log + Average_duration_of_fixations, data = subject_df)
...
dosw_dodw_study <- study_subject_norm_long[c(449:476,505:532),]
dosw_dodw_retrieval <- retrieval_subject_norm_long[c(449:476,505:532),]
