...
# placement_error_cm
ggscatter(myData_NO, x = "total_fix_dur_ms", y = "placement_error_cm_log", add = "reg.line", conf.int = TRUE,
          cor.coef = TRUE, cor.coeff.args = list(method = "pearson", label.x = 4000, label.sep = "\n"), 
          xlab = "Total duration of fixations", ylab = "Placement Error (log cm)") 

# filter out 0 duration of fixations and too long total fixation time
duration_filter <- myData_NO %>%
  filter(myData_NO$total_fix_dur_ms < 9000 & total_fix_dur_ms != 0)

#jpeg("eye_total_duration.jpeg", width = 4, height = 3, units = 'in', res = 300)
ggscatter(duration_filter, x = "total_fix_dur_ms", y = "placement_error_cm_log", add = "reg.line", conf.int = TRUE, size = 1,
          cor.coef = TRUE, cor.coeff.args = list(method = "pearson", label.x = 3050, label.y = 2.3, label.sep = "\n"), 
          xlab = "Total duration of fixations", ylab = "Placement Error (log cm)") 
#dev.off()

ggscatter(myData_NO, x = "fix_num", y = "placement_error_cm_log", add = "reg.line", conf.int = TRUE,
          cor.coef = TRUE, cor.coeff.args = list(method = "pearson", label.x = 10, label.sep = "\n"), 
          xlab = "Number of fixations", ylab = "Placement Error (log cm)") 
fixation_filter <- myData_NO %>%
  filter(myData_NO$fix_num < 17 & myData_NO$fix_num > 0)

#jpeg("eye_fixation_number.jpeg", width = 4, height = 3, units = 'in', res = 300)
ggscatter(fixation_filter, x = "fix_num", y = "placement_error_cm_log", add = "reg.line", conf.int = TRUE,
